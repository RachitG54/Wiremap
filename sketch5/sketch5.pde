...
static public void main(String args[]) {
  PApplet.main(new String[] { "--present", "sketch5" });
}
...
void draw() {
  ...
  int k;
  int alpha = 53;
  if(z_by_ind[alpha]>=2*depth/3)
  {
    k=2;
  }
  else if(z_by_ind[alpha]>=depth/3)
  {
    k=1;
  }
  ...
  for(int i = 0; i < alpha; i ++){
    if(map[i]>=2*80/3.0)
    fill(255,0,0);
    else if(map[i]>=80/3.0)
    fill(0,0,255);
    else fill(255,0,0);
   
  
  //println(x_by_ind[0]+" "+z_by_ind[0]+" "+x_by_ind[127]+" "+z_by_ind[127]);
  rect(i * width / wire, 0, 4, height);
  //rect(i*4, height - 40, 2, 40);
  }
  fill(0,255,0);
  rect(alpha * width / wire, 0, 4, height);
  
  println(depth-z_by_ind[alpha]);



  int alpha2=123;
  for(int i = 127; i > alpha2; i --){
   fill(255,0,0);
  ...
  fill(0,255,0);
  rect(alpha2 * width / wire, 0, 4, height);
  
  println(depth-z_by_ind[alpha2]);
