...
if(mousePressed)
  ...
  //constructor
  public Player(float x,float z,float r)
    {
   this.x=x;
   this.z=z;
   this.r=r;
    ...
    for(int i=0;i<wire;i++)
      {
        ...
        {
        fill(255,0,0);
        rect(i*width/wire,height-inPix(3,i),5,inPix(3,i));
  ...
  //constructor
    public Enemy(float x,float z,float r,float h)
      {
      this.x=x;
      this.z=z;
      this.r=r;
    ...
    public void move()//for moving it down,decrement h
    {
     if(h<=0) 
     h=heightMax;
    h=h-0.01;
    ...
    public void showEnemy()
    {
       for(int i=0;i<wire;i++)
      {
        ...
        {
        fill(255);
        rect(i*width/wire,height-inPix(h,i),5,inPix(r,i));
  ...
  public boolean Collide(Player P,Enemy E)
  {
    if((P.getX()-E.getX())*(P.getX()-E.getX())+(P.getZ()-E.getZ())*(P.getZ()-E.getZ())<(P.getR()+E.getR())*(P.getR()+E.getR()))//checking collision in x z plane
      if(E.getH()-3<E.getR()) // checking collision in Y direction
        return true;
      else 
        return false;
  ...
  Enemy en1=new Enemy(random(Xmax),random(Ymax),3,random(heightMax));
  Enemy en2=new Enemy(random(Xmax),random(Ymax),3,random(heightMax));
  Enemy en3=new Enemy(random(Xmax),random(Ymax),3,random(heightMax));
  Enemy en4=new Enemy(random(Xmax),random(Ymax),3,random(heightMax));   //creating 4 enemies at random positions//MEMORY ISN'T AN ISSUE,PLUS JAVA HAS GARBAGE COLLECTION
  
 void playGame1()
 {
     Player one=new Player(xMouse(),zMouse(),3);//defined inside because xMouse and zMouse should get updated
     //showing players
     one.showPlayer();
     en1.showEnemy();
     en2.showEnemy();
     ...
     if(Collide(one,en1)||Collide(one,en2)||Collide(one,en3)||Collide(one,en4))
     {
     //sound effects and flashes
     
     }
 }
 
 Player two=new Player(Xmid,Zmid,3);
