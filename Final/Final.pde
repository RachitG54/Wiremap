...
int   offsetsquare = 2;            
   ...
   float speedofrain = 0.2;
float speedofrain2 = 0.01;
boolean children = true;
float[] flag = new float[wire];
import processing.opengl.*;

static public void main(String args[]) {
  PApplet.main(new String[] { "--present", "Final" });
}
...
void sineSurface() {
  ...
  if(render_globe == true) {
    gen_globe(globe_x, -globe_y, globe_z, 8);
  }
...
void childrenlumarca()
{
  ...
  if(children==true)
    ...
    for(int i=0;i<wire;i++)
      {
     y_top_proj[i] = random(96);                      // compensate for projection morphing IN INCHES
     y_top_coord[i]=2+random(1);
     y_bot_coord[i] = 0;
     flag[i]=1;
     //println(r++);
       }
  ...
  for(int i=0;i<wire;i++)
  {
    ...
    {//y_top_proj = random(96);                      // compensate for projection morphing IN INCHES
     ...
     if(y_top_proj[i]<0)y_top_proj[i] = y_top_proj[i]+96;
     y_height_proj = 3;
...
//     amplitudeindex=1;
    ...
    // rect 3 is filler for sliver
    if(i%3==2)fill(colour,0,0);
    else if(i%2==1) fill(0,colour,0);
