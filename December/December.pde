...
float aspectrat         = 1.33333;            //aspect ratio for projector
...
ball moduleball = new ball();
pingpong2D moduleping = new pingpong2D();
crackers modulecracker = new crackers();
snakegame modulesnakegame = new snakegame();
mario modulemario = new mario();
...
void setup() {
  ...
  sunset1 = loadImage("sunsetsky.jpg");
  PVector v1 = new PVector(0.5,3);
  PVector spd = new PVector(0.08,-0.18,0);
  moduleball.initialize(0,12,depth - ((depth_levels/2)*depth_unit),4);
  moduleping.initialize(v1,spd);
  modulesine2.initialize();
  modulecracker.initialize();  
  modulemario.initialize();
  modulepattern1.initialize();
}
float t=1000;
int next = 0;
int countmodule=0;
void draw() {
  ...
  {
    modulecracker.create();
  }
  else if(next==4)
  {
    colorMode(RGB);// sets color mode value 
    modulesnakegame.create();
  }
  else if(next==5)
  {
    colorMode(RGB);// sets color mode value 
...
void keyReleased()
{
  ...
  {
    moduleping.keyreleased();
  }
  else if(next==5)
  {
    modulemario.keyreleased();
  }
}
void keyPressed()
{
  ...
  {
    moduleping.keypressed();
  }
  else if(next==4)
  {
    modulesnakegame.keypressed();
  }
  else if(next==5)
  {
    modulemario.keypressed();
  }
}
